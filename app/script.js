...
document.getElementById('pollutant-name').addEventListener('change', (event) => {
    ...
    // Показати додаткові поля тільки якщо вибрано "Інше" і таблиця "повітря"
...
document.getElementById('add-data-form').addEventListener('submit', (event) => {
    ...
    // Отримуємо значення hazard-class, якщо вибрано "custom"
    const hazardClass = selectedPollutant === 'custom' 
        ? document.getElementById('hazard-class').value 
        : null;

    // Отримуємо вибрану таблицю
    const selectedTable = document.getElementById('table-select1').value;

    // Якщо вибрана таблиця "повітря" і поле hazard-class не заповнене, показуємо повідомлення і припиняємо виконання
    ...
    // Якщо таблиця "вода", hazardClass має бути null
    if (selectedTable === 'water') {
        hazardClass = null;
    }

    // Розрахунок ставки податку
    ...
    .then(data => {
        ...
        // Очистити форму та показати повідомлення
        document.getElementById('add-data-form').reset();
        showNotification("Запис було успішно додано!", "green");

        // Сховати додаткові поля, якщо були показані
